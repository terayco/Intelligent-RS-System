...
def xuanran_seg(name, data_dir, save_dir, colormap):  # 实现渲染的函数，考虑到我们的需要一次只能渲染一张，pic_source一样为图片位置
...
# 批量渲染
def bitch_xuanran_seg(data_dir, save_dir, imgs):
    temps = list()
    for img in imgs:
        maps = dict()
        for i in range(4):
            maps[i] = generate_url + xuanran_seg(img, data_dir, save_dir, i)
        temps.append(maps)
    return temps
